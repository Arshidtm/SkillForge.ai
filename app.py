                    ...
                    with st.spinner("Fetching job data..."):
                        ...
                        if not st.session_state.fetch_job:
                            ...
                            st.session_state.docs = documentation(
                                st.session_state.fetch_job
                            )
                            embedding = get_embedding_model()
                            st.session_state.vector_store = get_vector_store(
                                st.session_state.docs, embedding
                            )
    ...
    if st.session_state.job_role_validated:
        ...
        if st.session_state.uploaded_file and not st.session_state.resume_processed:
            with st.spinner("Extracting the resume"):
                try:
                    st.session_state.extracted_resume = extract_resume(
                        st.session_state.uploaded_file
                    )
                    st.session_state.extracted_details = extract_details(
                        st.session_state.extracted_resume, nlp
                    )
...
if "vector_store" in st.session_state:
    try:
        ...
        if (
            ...
            and not st.session_state.resume_analyzed
            and st.session_state.extracted_details
        ):
